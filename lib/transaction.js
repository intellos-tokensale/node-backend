...
function saveAllTransactionsWAN(transactions) {
    return series.forEach(transactions, 0, tx => {
        return price.getByTime(tx.time)
            .then(prices => {
                return calcTransactionDataAndSave('WAN', prices.wanDollarPrice, tx.accountId, prices, tx);
            });
    })
}
...
function calcTransactionDataAndSave(currency, currencyPrice, accountId, prices, tx) {
    console.log(currency, currencyPrice, accountId, prices);
