...
function updateForBTCAddress(address, accountId) {
    if (!accountId) throw 'Transaction: accountId not defined';
    if (!address) throw 'Transaction: address not defined';
    if (!bitcoin_address.validate(address)) throw 'Transaction: Not a valid BTC address';
...
function upateForAllETHAddresses() {
...
function updateForETHAddress(address, accountId) {
    if (!accountId) throw 'Transaction: accountId not defined';
    if (!address) throw 'Transaction: address not defined';
    if (!ethereum_address.isAddress(address)) throw 'Transaction: Not a valid ETH address';
...
function calcTransactionDataAndSave(currency, currencyPrice, accountId, prices, tx) {
    ...
    var discount = _m.newD(prices.discount).add(_m.ONE);
    var value = tx.value;
    var dollarRate = _m.newD(currencyPrice);
    var dollarPrice = _m.newD(prices.dollarPrice);
