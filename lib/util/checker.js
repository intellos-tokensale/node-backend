...
export default {
    ...
    checkBlockchainRpcBTCTransactionProperties
};
...
function checkBlockchainRpcBTCTransactionProperties(tx) {
    if (!tx.amount) throw 'BTC: Transaction amount undefined';
    if (isNaN(parseFloat(tx.amount)) || !isFinite(tx.amount)) throw 'BTC: amount not a number';
    if (!tx.txid) throw 'BTC: Transaction txid undefined';

    if (!('confirmations' in tx)) throw 'BTC: Transaction confirmations undefined';
    if (isNaN(parseInt(tx.confirmations)) || !isFinite(tx.confirmations)) throw 'BTC: Transaction confirmations not a number';
    if (parseInt(tx.confirmations) < 0) throw 'BTC: Transaction confirmations negative';
