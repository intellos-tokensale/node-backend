...
const wanchain_address = require('wanchain-util');
...
function checkPricesProperties(prices) {
    ...
    if (!prices.wanDollarPrice) throw new Error('Price: for WAN not set');
...
function checkAccountProperties(account) {
    ...
    console.log(account);
    if (!account.wanAddress) throw new Error('Account: wanAddress missing');
    if (!wanchain_address.isValidAddress(account.wanAddress)) throw new Error('Account: Not a valid WAN address');
}
...
function priceRowPlausabilityCheck(row, lastaverage) {
    ...
    if (!row.wanDollarPrice) throw new Error('Plausabilty: WAN price not set in row');
    ...
    if (!lastaverage.wanDollarPrice) throw new Error('Plausabilty: WAN price not set in lastaverage');
    ...
    if (!(row.wanDollarPrice < lastaverage.wanDollarPrice * (1 + diff))) throw new Error('Plausabilty: wanprice has a too high value');
    if (!(row.wanDollarPrice > lastaverage.wanDollarPrice * (1 - diff))) throw new Error('Plausabilty: wanprice has a too low value');
    return row;
