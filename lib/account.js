...
function register(data) {
    if (!data.title) throw new Error('Account: no title defined');
...
function changePW(data) {
    if (!data.id) throw new Error('Account: no id defined');
    if (!data.password) throw new Error('Account: no password defined');

    data.hash = bcrypt.hashSync(data.password, bcrypt.genSaltSync(10));

    if (!data.referedByCode) referedByCode = null;
    return models.Accounts.find({ where: { id: data.id } })
        .then((acc) => {
            if (!acc) throw new Error('Email Not Register');
            return acc.updateAttributes({
                passwordHash: data.hash,
            });
        });
    ...
    return models.Accounts.find({ where: { email: email } })
        .then((acc) => {
            ...
            password = randomstring.generate({
                length: 8,
                charset: 'abcdefghjklmnopqrstuvwxyzABCDEFGHJKLNPQRSTUVWXYZ23456789$%+'
            });
        ...
        .then(acc => {
            checker.checkAccountProperties(acc);
            events.eventEmitter.emit(events.EVENT_PASSWORDRESET, acc.id, password);
            return acc;
        });
}
...
function confirmEmail(emailConfirmationCode) {
    ...
    return models.Accounts.find({ where: { emailConfirmationCode: emailConfirmationCode } })
        ...
        .then(acc => {
            events.eventEmitter.emit(events.EVENT_EMAILCONFIRMATION, acc.id);
            return acc;
        });
}
